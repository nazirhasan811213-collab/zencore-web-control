...
test('PostgreSQL preserves preferences and de-duplicates concurrent feed across restarts; opt-out cancels queued Telegram', {skip:!url},async()=>{
 ...
 try{
  ...
  const m={symbol:'XAUUSD',receivedAt:Date.now(),feedMode:'BAR-CLOSE',sidewaysGuard:false,strategyNormal:{state:'READY',side:'BUY',solid:true,sop:{feedReady:true,sopGreen:5,pricePastEntry:true,forecastPass:true,m5Pass:true},plan:{entry:2000,sl:1990,tp1:2010,tp2:2020,tp3:2030}}};
  ...
  assert.equal((await a.feed('0')).events.length,1,'WAIT bounce and changed SL must not enqueue duplicates across instances');
  ...
  await b.record({...m,receivedAt:m.receivedAt+6,strategyNormal:{state:'WAIT'}});
  await a.record({...m,receivedAt:m.receivedAt+7,positionManagement:{action:'CLOSE_50_NOW'}});
  assert.equal((await b.feed('0')).events.length,2,'close ledger persists across HOLD frames');
  // Two verified ZenCore users may share one Telegram chat: deliver only once.
  ...
  await a.record({...m,receivedAt:m.receivedAt+8,positionManagement:{action:'EXIT_ALL'}});
  await a.deliver();assert.equal(sends,1,'shared chat receives one message per event');
