...
test('hosted heartbeat casts reused status parameter consistently for PostgreSQL', async () => {
    ...
    query: async (sql, args) => {
      ...
      return {
        rows: [{
          ...
          verified_at: args[3] === 'CONNECTED_LOCKED' ? args[13] : null,
          created_at: args[13],
          updated_at: args[13]
        }]
      };
  ...
  assert.match(capturedSql, /status = \$4::varchar\(32\)/);
  assert.match(capturedSql, /CASE WHEN \$4::varchar\(32\) = 'CONNECTED_LOCKED'::varchar\(32\)/);
  assert.equal(capturedArgs[3], 'CONNECTED_LOCKED');
