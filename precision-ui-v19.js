...
function evaluate(){
  ...
  const sideways=!!p.sidewaysGuard,sidewaysReason=p.sidewaysReason||''; const conflict=!!p.predictionConflict; const predPass=!sideways&&(signalLocked?['SETUP_READY','ENTRY_READY','ACTIVE'].includes(signalStage):(side!=='WAIT'&&conf>=82&&cons>=6&&st>=70&&agree>=68&&!conflict));
  ...
  return{side,rawSide,rawConf,rawC,strategyFast:p.strategyFast||null,strategyNormal:p.strategyNormal||null,analysis3m:p.analysis3m||null,fastTrade1m:p.fastTrade1m||null,signalStage,signalLocked,signalReason:p.signalReason||'',sidewaysGuard:sideways,sidewaysReason,sidewaysChop:N(p.sidewaysChop),sidewaysEmaFlips:N(p.sidewaysEmaFlips)||0,sidewaysPriceReversals:N(p.sidewaysPriceReversals)||0,opportunityType:U(p.opportunityType||'NONE'),opportunitySide:U(p.opportunitySide||'WAIT'),opportunityStrength:U(p.opportunityStrength||'NONE'),opportunityReason:p.opportunityReason||'',opportunityRisk:U(p.opportunityRisk||'WAIT'),opportunityPrice:N(p.opportunityPrice),fresh,conf,cons,evidence,st,agree,conflict,predPass,c,confirmPass,ch,chopPass,planRR,rrPass,geo,z,price,inZone,nearZone,atTriggerArea,entryPass,exact,exactMatch,elite,quality,decision,tone,why,active};
}
const css=`
