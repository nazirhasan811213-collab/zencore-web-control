...
let pred=null,raw=null,lastRender=0;
...
function evaluate(){
  const p=pred||{},d=raw||{}; const side=U(p.prediction||'WAIT'); const fresh=freshness(d);
  const conf=N(p.predictionConfidence)||0,cons=N(p.predictionConsensus)||0,evidence=N(p.predictionEvidence)||0,st=N(p.stability)||0,agree=N(p.predictionAgreement)||0;
  const conflict=!!p.predictionConflict; const predPass=side!=='WAIT'&&conf>=82&&cons>=6&&st>=70&&agree>=68&&!conflict;
  ...
  else if(side==='WAIT'){decision='NO TRADE';why='Prediction belum mempunyai arah yang cukup kuat.';}
  else if(!predPass){decision=`WATCH ${side}`;tone=side.toLowerCase();why='Prediction ada arah tetapi belum lulus Precision Gate.';}
  ...
  return{side,fresh,conf,cons,evidence,st,agree,conflict,predPass,c,confirmPass,ch,chopPass,planRR,rrPass,geo,z,price,inZone,nearZone,atTriggerArea,entryPass,exact,exactMatch,elite,quality,decision,tone,why,active};
}
const css=`
...
function render(){if(!pred||!raw)return;lastRender=Date.now();const x=evaluate(),e=host();e.className=`v19${x.tone}`;const zoneText=x.inZone?'IN PINE ZONE':x.nearZone?'NEAR PINE ZONE':x.atTriggerArea?'NEAR ENTRY':'OUTSIDE EXECUTION';const confirmDetail=x.c.items.map(i=>`${i.name}:${i.pass?'✓':'×'}`).join('  ');e.innerHTML=`<div class="v19top"><div><div class="v19ey">ZENCORE V19 • PRECISION-FIRST DECISION ENGINE</div><div class="v19decision">${esc(x.decision)}</div><div class="v19sub">Prediction: <b>${esc(x.side)}</b> • ${esc(x.why)}</div></div><div class="v19score"><span>SETUP QUALITY</span><b>${x.quality}/100</b><small>Bukan win probability</small></div></div><div class="v19gates">${gate('1 • PREDICTION',x.predPass,`${x.side} ${x.conf}/100`,`Consensus ${x.cons}/${x.evidence} • Stability ${x.st}/100 • Agreement ${x.agree}%`)}${gate('2 • CONFIRMATION',x.confirmPass,`${x.c.passed}/${x.c.total} SEHALA`,confirmDetail)}${gate('3 • ENTRY TRIGGER',x.entryPass,zoneText,`Trigger ${x.exact}${x.exactMatch?' ✓':''} • R:R ${x.planRR==null?'—':x.planRR.toFixed(2)+'R'} • ${x.geo?'Geometry valid':'Plan mismatch'}`)}</div><div class="v19row"><div class="v19mini"><span>Feed</span><b>${esc(x.fresh)}</b></div><div class="v19mini"><span>Chop</span><b>${x.ch==null?'—':x.ch.toFixed(1)+'%'}</b></div><div class="v19mini"><span>MTF Align</span><b>${x.c.mtfAligned}/3</b></div><div class="v19mini"><span>Prediction</span><b>${esc(x.side)} ${x.conf}/100</b></div><div class="v19mini"><span>Execution</span><b>${esc(zoneText)}</b></div></div><div class="v19elite ${x.elite?'on':''}">${x.elite?'◆ ELITE FILTER LULUS — semua syarat precision utama sangat kuat. Tetap gunakan risk management.':'ELITE FILTER BELUM LULUS — ZenCore sengaja lebih selektif untuk kurangkan false signal.'}</div>`;}
...
boot();setInterval(()=>{if(Date.now()-lastRender>6000)boot()},6000);
})();
