...
The hosted-MT5 redesign is staged and disabled by default. The browser uses hybrid encryption (RSA-OAEP SHA-256 + AES-256-GCM); Render/PostgreSQL receive only ciphertext plus masked identity, and the private key is forbidden from the web tier. `hosted-mt5-worker/` now contains the connection-only runtime, Cloud KMS adapter, Google instance-identity transport, strict Demo MT5 telemetry adapter, Windows installer and reproducible artifact build with a hard-coded order gate set to `False`. `infra/gcp-hosted-mt5/` defines a private Google Cloud Windows Server 2022 Shielded VM, HSM-backed RSA-OAEP key, one-account cell assignment, keyless worker identity, IAP-only RDP and fail-closed bootstrap. Google Cloud Windows does not support Confidential VM, so role separation and the documented privileged-cloud-admin limitation apply. No Google resource is created by this repository code; a GCP project, reviewed artifacts, broker terminal and Demo broker matrix are still required before hosted execution can be considered.
