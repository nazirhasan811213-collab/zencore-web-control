...
- The trader owns the Windows PC or Azure subscription, Windows administrator access and encryption keys. ZenCore administrators receive none of them.
...
- Existing Windows PC: use `README-PC-WINDOWS.md` and install with `-HostProfile WINDOWS_PC`.
- Azure Confidential VM: use `infra/azure-trader-pod/README.md` and install with `-HostProfile AZURE_CONFIDENTIAL_VM`.

Both profiles write only a non-secret config, create a pinned Python environment and keep `demoExecutionEnabled=false`.

## One-time pairing

1. The signed-in trader opens `/auto-trade`, selects **JANA KOD PAIRING**, and types `PAIR SECURE POD`.
2. On the matching trader-owned Windows host, run `Pair-ZenCoreSecurePod.ps1` as the dedicated worker user.
