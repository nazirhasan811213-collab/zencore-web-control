...
function render(d){
  ...
  e.innerHTML=`<div class="v20head"><b>🛡️ SECURE PROFIT GUARD</b><span class="v20badge">${x.action}</span></div><div class="v20main"><div class="v20action"><span>Apa Nak Buat Sekarang</span><b>${x.title}</b><small>${x.msg}</small></div><div class="v20kpi"><span>Current R</span><b>${fmtR(x.current)}</b></div><div class="v20kpi"><span>Peak R</span><b>${fmtR(x.peak)}</b></div><div class="v20kpi"><span>Profit Bocor</span><b>${fmtR(x.giveback)}</b></div><div class="v20kpi"><span>Setup Check</span><b>${x.strong} kuat • ${x.warn} warning</b></div></div><div class="v20notes">${x.notes.length?x.notes.join(' • '):'Guard tengah monitor trade dan profit giveback.'}</div>`;
