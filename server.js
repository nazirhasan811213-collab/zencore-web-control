...
function serveFile(res, filename, type) {
  try {
    return send(res, 200, fs.readFileSync(path.join(__dirname, filename)), type);
  } catch (_) {
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === "GET" && pathname === "/debug/test-feed") {
    const now = Date.now();
    const sample = {
      source:"ZenCore AI Dashboard Pro + Alerts", symbol:"XAUUSD", tickerid:"OANDA:XAUUSD", timeframe:"1", tradeMode:"Scalping",
      time:now, open:3498.20, high:3502.10, low:3497.80, close:3500.25, volume:1840,
      action:"BUY", barStatus:"TEST FEED", marketStructure:"Bullish", momentum:"Bullish", demand:"Demand", hemaTrend:"Bullish",
      ema9:3499.6, ema20:3497.8, ema50:3492.4, hemaFast:3499.4, hemaSlow:3496.9, rsi:61.2, waveTrend1:22.4, waveTrend2:17.8,
      chopIndex:32.4, dxyStatus:"Weak", sdClearance:"Clear", whaleState:"Accumulation", relativeVolume:1.42,
      globalTrend:1, setupProbability:78, confluenceStars:4, riskState:"LOW", atr:4.25,
      bullObTop:3495.4, bullObBottom:3493.8, bearObTop:3508.2, bearObBottom:3506.7,
      entry:3500.25, sl:3495.00, tp1:3505.50, tp2:3510.75, tp3:3516.00,
      tradeActive:true, tradeIsBuy:true, tp1Hit:false, tp2Hit:false, tp3Hit:false, slHit:false,
      mtf1:1, mtf2:1, mtf3:1, mtfTotal:3, mtfOverall:"BULLISH", forecast3Bars:"Bullish continuation",
      marketPower:76, powerText:"BUYERS STRONG", proTip:"SERVER TEST FEED — replace with TradingView live data",
  ...
  if (req.method === "GET" && pathname === "/health")
    return send(res, 200, JSON.stringify({service:"zencore-web-control", ...webhookStatus()}));
  ...
  if (req.method === "GET" && pathname === "/api/journal")
    return send(res, 200, JSON.stringify(journal));

  if (req.method === "GET" && (pathname === "/" || pathname === "/index.html"))
    return serveFile(res, "index.html", "text/html; charset=utf-8");
  if (req.method === "GET" && pathname === "/style-v3.css")
    return serveFile(res, "style-v3.css", "text/css; charset=utf-8");
  if (req.method === "GET" && pathname === "/app-v3.js")
    return serveFile(res, "app-v3.js", "application/javascript; charset=utf-8");
...
server.listen(PORT, "0.0.0.0", () => {
  console.log(`ZenCore Total Analysis Terminal V3 running on port ${PORT}`);
});
