...
function proxyHtml(req,res,targetPath){
  ...
  const p=http.request({hostname:'127.0.0.1',port:V17_PORT,path:targetPath,method:'GET',headers},u=>{
    ...
    u.on('end',()=>{
      ...
      if(!html.includes('/precision-ui-v19.js'))html=html.replace('</body>','<script src="/precision-ui-v19.js?v=19.1"></script></body>');
...
server.listen(PUBLIC_PORT,'0.0.0.0',()=>{
  console.log(`ZenCore V25 Trader Decision Cockpit gateway running on port ${PUBLIC_PORT} -> V17 ${V17_PORT}`);
});
