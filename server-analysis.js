...
function proxyHtml(req,res,targetPath){
  ...
  const p=http.request({hostname:'127.0.0.1',port:V17_PORT,path:targetPath,method:'GET',headers},u=>{
    ...
    u.on('end',()=>{
      ...
      if(!html.includes('/v19-state-consistency.js'))html=html.replace('</body>','<script src="/v19-state-consistency.js?v=32.0"></script></body>');
      ...
      if(!html.includes('/strategy-isolation-v29.js'))html=html.replace('</body>','<script src="/strategy-isolation-v29.js?v=32.0"></script></body>');
      if(!html.includes('/final-layout-v30.js'))html=html.replace('</body>','<script src="/final-layout-v30.js?v=32.0"></script></body>');
      if(!html.includes('/setup-wake-v30-2.js'))html=html.replace('</body>','<script src="/setup-wake-v30-2.js?v=32.0"></script></body>');
      html=html.replace(/<title>[^<]*<\/title>/i,'<title>ZenCore V32 — Pine SOP Normal</title>');
...
server.listen(PUBLIC_PORT,'0.0.0.0',()=>{
  console.log(`ZenCore V32 Pine SOP Normal gateway running on port ${PUBLIC_PORT} -> V17 ${V17_PORT}`);
});
