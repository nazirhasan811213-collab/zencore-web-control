...
function proxyHtml(req,res,targetPath){
  ...
  const p=http.request({hostname:'127.0.0.1',port:V17_PORT,path:targetPath,method:'GET',headers},u=>{
    ...
    u.on('end',()=>{
      ...
      if(!html.includes('/interactive-layout-v24.js'))html=html.replace('</body>','<script src="/interactive-layout-v24.js?v=24.0"></script></body>');
      html=html.replace(/<title>[^<]*<\/title>/i,'<title>ZenCore V24 — Interactive Trader Dashboard</title>');
...
server.listen(PUBLIC_PORT,'0.0.0.0',()=>{
  console.log(`ZenCore V24 Interactive Trader Dashboard gateway running on port ${PUBLIC_PORT} -> V17 ${V17_PORT}`);
});
