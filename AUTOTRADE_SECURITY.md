...
Live-money execution remains locked. The legacy worker stays XAUUSD/InterStellar Demo only. Hosted connector `2.2.0-gcp-multiuser-multipair` accepts a reviewed subset of the 11 canonical symbols and 1–10 layers, but cannot place orders until the server gate, local gate, HSM key release, outbound-only private networking, broker symbol/server discovery, full Demo test matrix and independent security review pass.

The currently deployed `2.0.0-gcp-connect` worker is connection-only and remains locked. The `2.2.0-gcp-multiuser-multipair` source contains the guarded Demo order adapter, but it stays inert unless the reviewed server rollout gate, pinned artifact, local execution gate, assigned slot and runtime checks all pass. REAL accounts remain blocked.
