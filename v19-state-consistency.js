...
function predSide(){const s=U(pred?.signal||pred?.prediction);return /BUY|LONG|BULL/.test(s)?'BUY':/SELL|SHORT|BEAR/.test(s)?'SELL':'WAIT';}
...
function topState(d,f){const s=side(d),ps=predSide(),tp3=d?.tp3Hit===true,sl=d?.slHit===true&&!tp3,active=d?.tradeActive===true&&!tp3&&!sl;
  ...
  const td=translateDecision(rawDec);set('v10Decision',td,td.includes('BUY')?'#32e38d':td.includes('SELL')?'#ff6070':'#ffbf58');set('v10DecisionSub',rawSub||'Tunggu setup yang betul-betul cun.');const locked=!!pred?.signalLocked;set('v10Signal',ps==='WAIT'?'WAIT':locked?`SIGNAL ${ps} LOCKED`:`BIAS ${ps}`,ps==='BUY'?'#32e38d':ps==='SELL'?'#ff6070':'#ffbf58');
}
