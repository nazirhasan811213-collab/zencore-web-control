...
function brand(){const h=document.querySelector('header.top .brand h1');if(h)h.textContent='ZENCORE V20 — SECURE PROFIT ANALYSIS';const phases=document.querySelectorAll('header.top .phase-pill');if(phases.length)phases[phases.length-1].textContent='SECURE PROFIT';const sig=q('v10Signal')?.previousElementSibling;if(sig)sig.textContent='BIAS / TRADE';}
