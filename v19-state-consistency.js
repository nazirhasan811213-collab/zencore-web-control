...
function brand(){const h=document.querySelector('header.top .brand h1');if(h)h.textContent='ZENCORE V22.1 — STEP-BY-STEP TRADER VIEW';const phases=document.querySelectorAll('header.top .phase-pill');if(phases.length)phases[phases.length-1].textContent='6-STEP FLOW';const sig=q('v10Signal')?.previousElementSibling;if(sig)sig.textContent='BIAS / TRADE';}
