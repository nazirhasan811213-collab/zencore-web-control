(() => {
  ...
  document.addEventListener('pointerdown',()=>{try{audio ||= new (window.AudioContext||window.webkitAudioContext)();audio.resume().then(()=>{if(audio.state==='running')q('zaAudio').textContent='Bunyi sedia pada browser ini.';}).catch(()=>{});}catch{}},{once:true});
