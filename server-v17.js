...
function broadcastPrediction(symbol){require('./analysis-alert-bus').emit('market',marketSummary(symbol));const set=predictionClients.get(symbol);if(!set)return;const payload=JSON.stringify(marketSummary(symbol));for(const res of set){try{res.write(`event: prediction\ndata: ${payload}\n\n`)}catch(_){set.delete(res)}}}
